...
def insertTD(subj, table):
    with conn:
        connCur.execute(f"INSERT INTO {table} VALUES (:term, :definition)", {
                        'term': subj.term, 'definition': subj.definition})
...
def removeTD(term, table):
    with conn:
        connCur.execute(
            f"DELETE FROM {table} WHERE term = :term", {'term': term})
    getTD(table)
